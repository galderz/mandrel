...
public class InliningUtilities {
    ...
    private static boolean shouldBeTrivial(int numInvokes, int numOthers, StructuredGraph graph, CompileReason reason) {
        ...
        if (numInvokes == 0) {
            ...
            {
                System.out.printf(
                        "[%s][InliningUtilities.shouldBeTrivial][%s->%s] is leaf method, trivial check: number of non-invoke nodes %d <= %d? %b%n"
            ...
            if (numInvokes <= maxInvokesInTrivialMethod) {
                ...
                {
                    System.out.printf(
                            "[%s][InliningUtilities.shouldBeTrivial][%s->%s] is not leaf method && numInvokes(%d) <= MaxInvokesInTrivialMethod(%d) && numOthers(%d) <= MaxNodesInTrivialMethod(%d)? %b%n"
                            , Thread.currentThread().getName()
                            , reason == null ? "()" : reason.toString()
                            , methodId
                            , numInvokes
                ...
                {
                    System.out.printf(
                            "[%s][InliningUtilities.shouldBeTrivial][%s->%s] is not leaf method and not inlined (num invokes %d, MaxInvokesInTrivialMethod(%d), num others %d, MaxNodesInTrivialMethod(%d)%n"
                            , Thread.currentThread().getName()
                            , reason == null ? "()" : reason.toString()
                            , methodId
                            , numInvokes
    ...
    {
        return "String.charAt(int)".equals(caller)
    ...
    {
        return "String.isLatin1()".equals(methodId)
